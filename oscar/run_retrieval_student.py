...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, split='train', is_train=True):
        ...
        self.caption_ids = self.get_caption_ids()
    ...
    def get_caption_ids(self):
        print(len(self.captions), 'images found!')

        caption_ids = []
        for img_id, img_key in enumerate(self.img_keys):
            assert len(self.captions[img_key]) > 0, img_key
            for i in range(len(self.captions[img_key])):
                caption_ids.append((img_id, i))
    ...
    def get_image_caption_index(self, index):
        ...
        # print('>', len(self.caption_ids), index)

        # Make all image, caption pairs
        if not self.is_train and self.args.cross_image_eval:
            img_idx = index // len(self.caption_ids)
            cap_idx = index % len(self.caption_ids)
            img_idx1, cap_idx1 = self.caption_ids[cap_idx]
            assert 0 <= cap_idx1 and cap_idx1 < len(self.captions[self.img_keys[img_idx1]]), (img_idx1, cap_idx1, len(self.caption_ids), cap_idx, self.caption_ids[:cap_idx + 1], len(self.captions[self.img_keys[img_idx1]]), self.captions[self.img_keys[img_idx1]])
            return img_idx, [self.img_keys[img_idx1], cap_idx1]

        # Fixed number of sampled indices, for eg. 20 
        if not self.is_train and self.has_caption_indexs:
            img_idx = index // self.num_captions_per_img
            cap_idx = index % self.num_captions_per_img
            img_key1, cap_idx1 = self.caption_indexs[self.img_keys[img_idx]][cap_idx]
            assert 0 <= cap_idx1 and cap_idx1 < len(self.captions[img_key1])
            return img_idx, [img_key1, cap_idx1]

        assert 0 <= index and index < len(self.caption_ids)
        img_idx, cap_idx = self.caption_ids[index]
        assert 0 <= cap_idx and cap_idx < len(self.captions[self.img_keys[img_idx]])
        return img_idx, [self.img_keys[img_idx], cap_idx]
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, 
        ...
        if text_b and self.max_seq_len - seq_a_len >= 1:
        ...
        assert input_ids.shape[0] == 70, (text_a, text_b, len(tokens))
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            if random.random() <= 0.5:
                ...
                assert len(self.captions[self.img_keys[img_idx_neg]]) > 0
                cap_idx_neg = random.randint(0, len(self.captions[self.img_keys[img_idx_neg]]) - 1)
    ...
    def __len__(self):
        if not self.is_train and self.args.cross_image_eval:
            return len(self.img_keys) * len(self.caption_ids)
        if not self.is_train and self.has_caption_indexs:
...
def compute_ranks(dataset, results):
    ...
    if dataset.has_caption_indexs:
        ...
        num_captions_per_img = len(dataset.caption_ids)
...
def save_checkpoint(model, tokenizer, args, epoch, global_step, checkpoint_name=None):
    checkpoint_dir = op.join(args.output_dir, 'checkpoint-{}-{}'.format(
        epoch, global_step))
    if checkpoint_name is not None:
        checkpoint_dir = op.join(args.output_dir, checkpoint_name)
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    epoch_steps = len(train_dataloader) // args.gradient_accumulation_steps
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    ...
                    epoch_time = (time.time() - start_time) / (step + 1) * epoch_steps
                    print(f'{epoch_time // 60} minutes and {epoch_time % 60} seconds')
    ...
    save_checkpoint(model, tokenizer, args, args.num_train_epochs - 1, global_step, checkpoint_name='checkpoint-last') 
...
def main():
    ...
    parser.add_argument("--num_captions_per_img_train", default=None, type=int,
                        help="number of positive matched captions for each training image.")
    parser.add_argument("--num_captions_per_img_val", default=None, type=int,
    ...
    parser.add_argument("--do_kd", action='store_true', help="Perform knowledge distillation.")
    parser.add_argument("--lang", default='en', type=str, help="Target Language for training.")
    parser.add_argument("--only_classifier", default=False, action='store_true', help="Train only the classifier while the rest of the model is frozen.")
