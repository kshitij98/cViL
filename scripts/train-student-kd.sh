...
if [ "$ONLY_CLASSIFIER" == "0" ]; then
    ...
    python3 oscar/run_vqa_student.py -j 8 \
        ...
        --num_train_epochs 10 \
        --output_dir $MODEL_DIR/kd-code_mix \
...
if [ "$ONLY_CLASSIFIER" == "1" ]; then
    echo "Training classifier...!!! for $EXPERIMENT_ID"
    ...
    python3 oscar/run_vqa_student.py -j 8 \
        ...
        --teacher_features_dir /ssd_scratch/cvit/$USER \
        --fp16 \
        --model_name_or_path $MODEL_DIR/distiled+classifier/best \
        --learning_rate 5e-05 \
        --lang ja \
        --only_classifier \
        # --do_kd \
