...
export MODEL_DIR=$MODELS_DIR/vinvl-base/vg

mv $DATASET_DIR/test-dev2015_qla_mrcnn.json.unknown $DATASET_DIR/test-dev2015_qla_mrcnn.json
...
MODELS="ja/ours-kd/vg ja/ours-aug/vg ja/baseline ja/ablations/codemix/vg ja/ablations/inter/vg ja/ablations/cls/vg ja/ablations/images/vg ja/ablations/tags/vg"
...
for MODEL_NAME in $MODELS; do
    ...
    if [ -f "$MODEL_DIR/$MODEL_NAME/test-dev2015_results.json" ]; then
        echo "Already calculated on $MODEL_NAME"
        continue
    fi
    ...
    python3 ~/scripts/jap-vg-score.py $MODEL_DIR/$MODEL_NAME/test-dev2015_results.json
